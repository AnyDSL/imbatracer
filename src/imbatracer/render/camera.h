...
public:
    ...
    void sample_pixel(int x, int y, ::Ray& ray_out, RNG& rng) {
        const float sample_x = static_cast<float>(x) + rng.random_float();
        const float sample_y = static_cast<float>(y) + rng.random_float();
