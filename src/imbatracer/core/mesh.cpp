...
void Mesh::compute_normals(bool smooth, int normal_attr) {	
	if (smooth) {
		...
		for (int i = 0; i < triangle_count(); ++i) {
			...
			normals[indices_[i * 3 + 0]] += n;
            normals[indices_[i * 3 + 1]] += n;
            normals[indices_[i * 3 + 2]] += n;
		}
		...
		for (int k = 0; k < attribute_count(); ++k) {
			...
			for (int i = 0; i < triangle_count(); ++i) {
				const int i0 = indices_[i * 3 + 0];
                const int i1 = indices_[i * 3 + 1];
                const int i2 = indices_[i * 3 + 2];
